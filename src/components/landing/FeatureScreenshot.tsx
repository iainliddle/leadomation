...
const FeatureScreenshot: React.FC = () => {
  return (
    <section className="py-24 bg-linear-to-b from-white from-50% to-gray-100">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-start">
              ...
              {features.map((f) => (
                <div key={f.title} className="lp-fade">
                  <dt className="flex items-start gap-3">
                    ...
                    <span className="text-base font-semibold text-gray-950">{f.title}</span>
                  </dt>
                  <dd className="mt-2 ml-11 text-sm text-gray-500 leading-relaxed">{f.desc}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="mt-12 lg:mt-0">
            <div className="rounded-[2rem] bg-white shadow-xl ring-1 ring-black/5 overflow-hidden">
              <div className="p-6">
                {/* Search bar */}
                ...
                <div className="flex gap-2 mb-4">
                  {[
                    { label: 'Hot', bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-100' },
                    { label: 'Warm', bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-100' },
                    { label: 'Cool', bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100' },
                    { label: 'Unscored', bg: 'bg-gray-50', text: 'text-gray-500', border: 'border-gray-100' },
                  ].map((pill) => (
                    <span
                      key={pill.label}
                ...
                <div className="space-y-0 divide-y divide-gray-100">
                  {[
                    { name: 'Dunmore Dental Care', status: 'ENRICHED', action: 'CONTACTED', intent: 'Hot', score: 63 },
                    { name: 'Smile Clinic Northwest', status: 'ENRICHED', action: 'REPLIED', intent: 'Hot', score: 95 },
                    { name: 'Bright Smile Kent', status: 'ENRICHED', action: 'CONTACTED', intent: 'Hot', score: 76 },
                  ].map((row) => (
                    <div key={row.name} className="flex items-center justify-between py-3">
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{row.name}</p>
                        <div className="flex gap-2 mt-1">
                          <span className="text-[10px] font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">{row.status}</span>
                          <span className="text-[10px] font-medium text-[#4F46E5] bg-[#EEF2FF] px-2 py-0.5 rounded">{row.action}</span>
                        </div>
                      </div>
